...
from utile import is_legal_move
from utile import has_tile_to_flip
from networks_2204660 import MLP,LSTMs
...
class CustomDataset(Dataset):
    def __init__(self,
        ...
        self.game_files_name=list_files[:500]#[s + ".h5" for s in list_files]       #prendre seulement 500 echantillons
    ...
    def __getitem__(self, idx, player= -1):
        ...
        if self.samples[idx].end_move+1 >= self.samples[idx].len_moves:
            #if is_legal_move(end_move,self.starting_board_stat,player):
...
print(model)
opt = torch.optim.Adadelta(model.parameters(), lr=0.02)
print(opt)
